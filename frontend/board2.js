...
function get_data(){
    const url = 'http://localhost:5050/tabla';
     fetch(url).then((data) => data.json().then((jsonData) => renderBoard(jsonData)))  //aici in loc de return data.json ar trebui apelata functia care sa faca ceva cu jsonul
        .then(res=>{console.log(res)})
         .catch(error=>{console.log(error)})
}
